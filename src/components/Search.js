...
export const Search=(props)=>{
    ...
    <div className="search-container w-full ml-10 pl-5 ">
        <input type="text" placeholder="Search" value={searchText} onChange={(e)=>{setSearchText(e.target.value)}} className="border-2 border-red-800 rounded-full px-3"></input>
        <button className="btn rounded-full bg-red-300 border-2 border-red-800 hover:bg-orange-600 m-2 px-2" onClick={()=>filterdata(searchText)} >Search</button>  {/*Always use callback/anonymous function inside onClick */}
    </div>);
}